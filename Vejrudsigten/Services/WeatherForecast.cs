...
namespace Vejrudsigten.Services
{
    public static class WeatherForecast
    {
        ...
        public static String GetWeatherMessageBasedOnInput(String town, String todaysConditions,
            double todaysTemperature,
            String yesterdayCondition, double yesterdayTemperature)
        {
            String result = "";

            if (todaysTemperature < 0 && todaysConditions.Contains(sne))
            {
                result = "Det bliver koldt med {0} grader og der kommer {1} vejr i {2}. I går var det {3} og {4} grader";
                return String.Format(result, todaysTemperature, todaysConditions, town, yesterdayCondition,
            ...
            if (todaysConditions.Contains(klartVejr) && yesterdayCondition.Contains(sne))
            {
                result = "Kælk og {0} vejr med {1} er forbi. Nu bliver det flot {2} med {3} i {4}";
                return String.Format(result, yesterdayCondition, yesterdayTemperature, todaysConditions,
                    todaysTemperature, town);
            }

            if (todaysConditions.Contains(klartVejr))
            {
                result = "Flot {0} i {1} med {2} grader. I går var det {3} med {4} grader";
                return String.Format(result, todaysConditions, town, todaysTemperature, yesterdayCondition,
                    yesterdayTemperature);
            }

            if (todaysConditions.Contains(regn))
            {
                result = "Nu kommer der {0} i {1} med {2} grader idag og i går var det {3} med {4} grader";
                return String.Format(result, todaysConditions, town, todaysTemperature, yesterdayCondition,
                    yesterdayTemperature);
            }

            if (todaysConditions.Contains(skyet) && yesterdayCondition.Contains(sne))
            {
            ...
            {
                result = "{0} vejr i {1} med {2} grader. I går var det {3} og {4} grader";
                return String.Format(result, todaysConditions, town, todaysTemperature, yesterdayCondition,
                    yesterdayTemperature);
            }
            ...
            return String.Format(result, town, todaysConditions, todaysTemperature, yesterdayCondition,
                yesterdayTemperature);
        }
    }
}